...
namespace VocalUtau.Wavtools.BPlayer
{
    public partial class Form1 : Form
    {
        ...
        private void InitPlayer(long headSize)
        {
            ...
            bplayer = new AppWrapper("BPlayer", AppWrapper.WrapperType.Buffer_Player);
            bplayer.CreateObject(new object[] { Fs, headSize + (long)fh.Ms2Bytes(delaybufftime) });
            bplayer.getInstanceObject<BufferedPlayer>().InitPlayer();
            bplayer.getInstanceObject<BufferedPlayer>().Buffer_Play();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            bplayer.getInstanceObject<BufferedPlayer>().DisposePlayer();
            bplayer.Unload();
        ...
        {
            int bfb = (int)((bplayer.getInstanceObject<BufferedPlayer>().BufferPercent / 0.8) * 100);
            progressBar1.Value = bfb < 100 ? bfb : 100;
            int bf2 = (int)((bplayer.getInstanceObject<BufferedPlayer>().UntallPercent / 0.9) * 100);
            progressBar2.Value = bf2 < 100 ? bf2 : 100;
            int bf3 = (int)((bplayer.getInstanceObject<BufferedPlayer>().StreamPercent / 1) * 100);
            progressBar3.Value = bf3 < 100 ? bf3 : 100;
            FormatHelper fh = new FormatHelper(IOHelper.NormalPcmMono16_Format);
            bplayer.getInstanceObject<BufferedPlayer>().FillBuffer(fh.Ms2Bytes(prebufftime));
        }
    }
}
